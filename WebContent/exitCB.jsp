...
<form class="context" action="exitCBCheck.jsp">
	<%
		
		String manager = clipboardDAO.getManager(cbid);
		String involved = clipboardDAO.getInvolvedExceptMe(cbid, userName);
		if(manager.equals( userName ) && !involved.equals("")) {
	%>
	<input type="hidden" name="formCbid" value="<%=cbid %>" >
	<input type="hidden" name="delegate" value="yes">
<%= userName %> 님은 <%= cbname %> 클립보드의 매니저입니다.<br/> 위임할 참가자를 선택해주세요.<br/>
위임 가능한 참가자 목록 : <%= involved %> <br/>
매니저 권한을 위임할 참가자 : <input type="text" name="nominee"><br/><br/>
	<%
		} else {
	%>
	클립보드 이름 : <%=cbname %> <input type="hidden" name="formCbid" value="<%=cbid %>" ><br/>
	<input type="hidden" name="delegate" value="no">
	<%
		}
	%>

	정말로 클립보드에서 퇴장하시겠습니까?<br/>
